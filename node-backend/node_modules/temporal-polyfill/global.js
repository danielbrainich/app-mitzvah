!function() {
  ...
  function formatPlainTimeIso(slots, options) {
    return ((fields, roundingMode, nanoInc, subsecDigits) => formatIsoTimeFields(roundTimeToNano(fields, nanoInc, roundingMode)[0], subsecDigits))(slots, ...refineTimeDisplayOptions(options));
  }
  ...
  function diffRelativeMarkers(largestUnit, slots0, slots1, calendarOps, timeZoneOps) {
    return timeZoneOps ? ((calendarOps, timeZoneOps, slots0, slots1, largestUnit) => {
    ...
    })(calendarOps, slots0, slots1, largestUnit);
  }
  ...
  function spanDuration(durationFields0, durationFields1, largestUnit, marker, calendarOps, timeZoneOps) {
    ...
    return durationFields1 && (endMarker = moveRelativeMarker(durationFields1, endMarker, calendarOps, timeZoneOps)), 
    [ diffRelativeMarkers(largestUnit, marker, endMarker, calendarOps, timeZoneOps), relativeMarkerToEpochNano(endMarker, timeZoneOps) ];
  }
  ...
  const expectedInteger = (entityName, num) => `Non-integer ${entityName}: ${num}`, expectedPositive = (entityName, num) => `Non-positive ${entityName}: ${num}`, expectedFinite = (entityName, num) => `Non-finite ${entityName}: ${num}`, forbiddenBigIntToNumber = entityName => `Cannot convert bigint to ${entityName}`, invalidBigInt = arg => `Invalid bigint: ${arg}`, forbiddenSymbolToString = "Cannot convert Symbol to string", invalidObject = "Invalid object", numberOutOfRange = (entityName, val, min, max, choices) => choices ? numberOutOfRange(entityName, choices[val], choices[min], choices[max]) : invalidEntity(entityName, val) + `; must be between ${min}-${max}`, invalidEntity = (fieldName, val) => `Invalid ${fieldName}: ${val}`, missingField = fieldName => `Missing ${fieldName}`, forbiddenField = fieldName => `Invalid field ${fieldName}`, duplicateFields = fieldName => `Duplicate field ${fieldName}`, noValidFields = validFields => "No valid fields: " + validFields.join(), invalidChoice = (fieldName, val, choiceMap) => invalidEntity(fieldName, val) + "; must be " + Object.keys(choiceMap).join(), forbiddenEraParts = "Forbidden era/eraYear", mismatchingEraParts = "Mismatching era/eraYear", mismatchingYearAndEra = "Mismatching year/eraYear", invalidEra = era => `Invalid era: ${era}`, missingYear = allowEra => "Missing year" + (allowEra ? "/era/eraYear" : ""), invalidMonthCode = monthCode => `Invalid monthCode: ${monthCode}`, mismatchingMonthAndCode = "Mismatching month/monthCode", missingMonth = "Missing month/monthCode", invalidLeapMonth = "Invalid leap month", invalidProtocolResults = "Invalid protocol results", mismatchingCalendars = "Mismatching Calendars", invalidCalendar = calendarId => `Invalid Calendar: ${calendarId}`, mismatchingTimeZones = "Mismatching TimeZones", forbiddenIcuTimeZone = "Forbidden ICU TimeZone", outOfBoundsOffset = "Out-of-bounds offset", outOfBoundsDstGap = "Out-of-bounds TimeZone gap", invalidOffsetForTimeZone = "Invalid TimeZone offset", ambigOffset = "Ambiguous offset", outOfBoundsDate = "Out-of-bounds date", outOfBoundsDuration = "Out-of-bounds duration", forbiddenDurationSigns = "Cannot mix duration signs", invalidLargeUnits = "Cannot use large units", flippedSmallestLargestUnit = "smallestUnit > largestUnit", failedParse = s => `Cannot parse: ${s}`, invalidSubstring = substring => `Invalid substring: ${substring}`, invalidFormatType = branding => `Cannot format ${branding}`, forbiddenFormatTimeZone = "Cannot specify TimeZone", mapPropNamesToIndex = bindArgs(mapPropNames, ((_propVal, i) => i)), mapPropNamesToConstant = bindArgs(mapPropNames, ((_propVal, _i, constant) => constant)), padNumber2 = bindArgs(padNumber, 2), unitNameMap = {
    ...
    dangi: 13,
    hebrew: -6
  }, RawDateTimeFormat = Intl.DateTimeFormat, isoTimeFieldNamesAsc = [ "isoNanosecond", "isoMicrosecond", "isoMillisecond", "isoSecond", "isoMinute", "isoHour" ], isoDateFieldNamesAsc = [ "isoDay", "isoMonth", "isoYear" ], isoDateTimeFieldNamesAsc = [ ...isoTimeFieldNamesAsc, ...isoDateFieldNamesAsc ], isoDateFieldNamesAlpha = sortStrings(isoDateFieldNamesAsc), isoTimeFieldNamesAlpha = sortStrings(isoTimeFieldNamesAsc), isoDateTimeFieldNamesAlpha = sortStrings(isoDateTimeFieldNamesAsc), isoTimeFieldDefaults = mapPropNamesToConstant(isoTimeFieldNamesAlpha, 0), maxMilli = 1e8 * milliInDay, epochNanoMax = [ 1e8, 0 ], epochNanoMin = [ -1e8, 0 ], isoYearMax = 275760, isoYearMin = -271821, isoEpochOriginYear = 1970, isoEpochFirstLeapYear = 1972, isoMonthsInYear = 12, primaryJapaneseEraMilli = isoArgsToEpochMilli(1868, 9, 8), queryJapaneseEraParts = memoize((isoFields => {
    const epochMilli = isoToEpochMilli(isoFields);
  ...
  const timeZoneNameStrs = [ "timeZoneName" ], monthDayFallbacks = {
    ...
    return timeZoneId;
  } ], emptyOptions = {}, slotsMap = new WeakMap, getSlots = slotsMap.get.bind(slotsMap), setSlots = slotsMap.set.bind(slotsMap), DateTimeFormat = function() {
  ...
  }, prepPlainYearMonthFormat = createFormatPrepper(plainYearMonthConfig), prepPlainMonthDayFormat = createFormatPrepper(plainMonthDayConfig), prepPlainDateFormat = createFormatPrepper(plainDateConfig), prepPlainDateTimeFormat = createFormatPrepper(plainDateTimeConfig), prepPlainTimeFormat = createFormatPrepper(plainTimeConfig), prepInstantFormat = createFormatPrepper(instantConfig), prepZonedDateTimeFormat = createFormatPrepper(zonedDateTimeConfig), yearMonthOnlyRefiners = {
    era(input) {
      if (void 0 !== input) {
    ...
    hoursInDay: slots => ((getTimeZoneOps, zonedDateTimeSlots) => {
    ...
    round: (slots, options) => createZonedDateTime(((getTimeZoneOps, zonedDateTimeSlots, options) => {
    ...
    startOfDay: slots => createZonedDateTime(((getTimeZoneOps, zonedDateTimeSlots) => {
      let {epochNanoseconds: epochNanoseconds, timeZone: timeZone, calendar: calendar} = zonedDateTimeSlots;
    ...
    round: (slots, options) => createPlainTime(((slots, options) => createPlainTimeSlots(((isoFields, smallestUnit, roundingInc, roundingMode) => roundTimeToNano(isoFields, computeNanoInc(smallestUnit, roundingInc), roundingMode)[0])(slots, ...refineRoundOptions(options, 5))))(slots, options)),
    equals(slots, other) {
    ...
    round: (slots, options) => createPlainDateTime(((plainDateTimeSlots, options) => createPlainDateTimeSlots(roundDateTime(plainDateTimeSlots, ...refineRoundOptions(options)), plainDateTimeSlots.calendar))(slots, options)),
    equals(slots, otherArg) {
    ...
    toString: (slots, options) => formatPlainDateTimeIso(slots, options),
    toJSON: slots => formatPlainDateTimeIso(slots),
    toLocaleString(slots, locales, options) {
      const [format, epochMilli] = prepPlainDateTimeFormat(locales, options, slots);
      return format.format(epochMilli);
    },
    toZonedDateTime: (slots, timeZoneArg, options) => createZonedDateTime(((getTimeZoneOps, plainDateTimeSlots, timeZoneSlot, options) => createZonedDateTimeSlots(((getTimeZoneOps, timeZoneSlot, isoFields, options) => {
    ...
    })(getTimeZoneOps, timeZoneSlot, plainDateTimeSlots, options), timeZoneSlot, plainDateTimeSlots.calendar))(createTimeZoneOps, slots, refineTimeZoneSlot(timeZoneArg), options)),
